...
const (
	...
	typeMissMatchError      = "unknown operator"
	identifierNotFoundError = "identifier not found"
	notFunctionError        = "unknown operator"
...
func TestErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{`{"name": "Monky"}[fn(x) { x }];`, fmt.Sprintf("unusable as hash key: %s", object.FUNCTION_OBJ)},
	}
...
func TestBuiltinFunctions(t *testing.T) {
	tests := []struct {
		...
		{`len("four")`, 4},
		{`len("hello world")`, 11},
		{`len(1)`, fmt.Sprintf("argument to `%s` not supported, got %s", object.BuiltinFuncNameLen, object.INTEGER_OBJ)},
		{`len("one", "two")`, "wrong number of arguments. got=2, want=1"},
		{`first([1, 2, 3])`, 1},
		{`first([])`, nil},
		{`first(1)`, fmt.Sprintf("argument to `%s` must be %s, got %s", object.BuiltinFuncNameFirst, object.ARRAY_OBJ, object.INTEGER_OBJ)},
		{`first(1, 2)`, "wrong number of arguments. got=2, want=1"},
		{`last([1, 2, 3])`, 3},
		{`last([])`, nil},
		{`last(1)`, fmt.Sprintf("argument to `%s` must be %s, got %s", object.BuiltinFuncNameLast, object.ARRAY_OBJ, object.INTEGER_OBJ)},
		{`last(1, 2)`, "wrong number of arguments. got=2, want=1"},
		{`rest([1, 2, 3])`, []int{2, 3}},
		{`rest([])`, nil},
		{`rest(1)`, fmt.Sprintf("argument to `%s` must be %s, got %s", object.BuiltinFuncNameRest, object.ARRAY_OBJ, object.INTEGER_OBJ)},
		{`rest(1, 2)`, "wrong number of arguments. got=2, want=1"},
		{`push([], 1)`, []int{1}},
		{`push(1, 2)`, fmt.Sprintf("argument to `%s` must be %s, got %s", object.BuiltinFuncNamePush, object.ARRAY_OBJ, object.INTEGER_OBJ)},
